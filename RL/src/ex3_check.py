...
def solve(game: ext_plant.Game, run_idx: int, controller_module):
    ...
    # Include the seed used for this run in the printed summary
    seed_used = game.get_problem().get("seed", "?")

    print(
        f"Run {run_idx:2d} (seed={seed_used}): {BOLD}{YELLOW}Reward: {r:3d}{RESET} | Steps: {game.get_max_steps():2d} | {color}{finished_txt}{RESET} \nState: {state_str}"
    )
    return r
...
def main():
    ...
    estimated_baseline_map = {
        ...
        # Estimated baselines for problem1..problem8 (from Assignment2 baseline_map)
        "problem1": (50.0, 5.0),
        "problem2": (120.0, 10.0),
        "problem3": (160.0, 2.0),
        "problem4": (65.0, 15.0),
